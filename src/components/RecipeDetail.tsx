...
const RecipeDetail: React.FC<RecipeDetailProps> = ({ match }) => {
  ...
  const renderRecipeDetails = () => {
    if (recipe) {
      return (
        <>
          <h2 className="text-4xl font-heading text-center underline uppercase text-primary mb-6">{recipe.name}</h2>
          <h3 className="text-xl mb-4">Ingredience:</h3>
          <div className="flex justify-around">
            {recipe.allIngredients.map((ingGroup, index) => (
              <div key={index} className="bg-secondary rounded-md text-white font-thin mr-4 p-2 text-lg w-1/3">
